...
// KV keys
const CURSOR_KEY      = "run_cursor_v4";     // global league index
const RUN_STATE_KEY   = "run_state_v4";      // per-league stage/progress
...
async function r2GetJSON(bucket, key, fallback = {}) {
  ...
  // R2.GET in Pages returns R2ObjectBody; need .text() not .body
...
export async function generateAll(env, log = () => {}, isCanceled = () => false) {
  ...
  // players DB ensure-once
  let playersObj = await env.LEADERBOARDS.get("sleeper_players.json");
  if (!playersObj) {
    const playersDBFresh = await fetchWithRetry("https://api.sleeper.app/v1/players/nfl", RETRIES, fetch, signal);
    await put("sleeper_players.json", JSON.stringify(playersDBFresh));
    playersObj = await env.LEADERBOARDS.get("sleeper_players.json");
  }
  const playersDB = JSON.parse(await (await env.LEADERBOARDS.get("sleeper_players.json")).text());

  // state helpers
  const yieldPause = async (stateToPersist) => {
    if (stateToPersist) await kvPutJSON(env.CONFIG_KV, RUN_STATE_KEY, stateToPersist);
  ...
  const weeklyKeyFor = (yy, cc, idx) => `weekly/${yy}/${cc}/part${idx}.json`;
  ...
  if (state.stage === "init") {
    ...
    log(`Stage: init → week(1)`);
    await yieldPause(state);
  }

  // ---- WEEK (one week per request) ----
  if (state.stage === "week") {
    const base = `https://api.sleeper.app/v1/league/${w.leagueId}`;
    ...
    if (!ms || !ms.length) {
      state.stage = "totals";
      log(`Stage: week → totals (no more weeks; last was ${state.week - 1})`);
      await yieldPause(state);
    }

    // build ONLY this week
    const weeklyRosters = [];
    ...
    // FULL shard merge for this week
    ...
    // WEEKLY shard (append this week only)
    ...
    if (sizeOfJSON(attempt) <= MAX_CHUNK) {
      ...
      await put(weeklyKey(partIdx), JSON.stringify(shardWeeklyAdd));
    }

    // remember latest roster snapshot
    state.lastRoster = { week: state.week, data: weeklyRosters };
    state.week += 1;

    log(`Stage: week → week(${state.week})`);
    await yieldPause(state);
  }
  ...
  if (state.stage === "latest") {
    ...
    state.stage = "advance";
    log(`Stage: latest → advance`);
    await yieldPause(state);
  }

  // ---- ADVANCE (move to next league) ----
  if (state.stage === "advance") {
    ...
    await env.CONFIG_KV.delete(RUN_STATE_KEY);

    const done = nextI >= work.length;
    log(done ? `All leagues processed.` : `Advance → league #${nextI+1}/${work.length}`);
