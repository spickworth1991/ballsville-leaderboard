...
import { createRun, getRun, endRun } from "../_lib/run-state";
...
export async function onRequestGet(context) {
  ...
  const { id, state } = createRun(); // <-- create a run record
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const logs = [];
        const { manifest } = await generateAll(env, (msg) => {
          logs.push(msg);
          // throttle a little if you want; here we stream every line
          send({ type: "log", msg });
        }, isCanceled);
        ...
        // If canceled, surface a friendly message
